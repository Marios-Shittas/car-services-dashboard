  ...
  return (
    <div className="grid gap-6">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div><h1 className="text-2xl font-bold">Vehicles</h1><p className="text-sm text-muted-foreground">Search by plate, VIN, brand, or model.</p></div>
        <form className="grid w-full grid-cols-[minmax(0,1fr)_40px] gap-2 lg:w-96"><Input name="q" defaultValue={q} placeholder="Plate, VIN, brand" /><Button size="icon"><Search className="h-4 w-4" /></Button></form>
      </header>
      <div className="grid gap-4 xl:grid-cols-[420px_1fr]">
        ...
        <Card><CardContent className="overflow-x-auto p-0"><Table><thead><tr><Th>Vehicle</Th><Th>Customer</Th><Th>VIN</Th><Th>Kilometers</Th><Th>Jobs</Th></tr></thead><tbody>
          {visibleVehicles.map((vehicle) => <tr key={vehicle.id}><Td><Link className="font-semibold text-primary" href={`/vehicles/${vehicle.id}`}>{vehicle.licensePlate}</Link><p className="text-xs text-muted-foreground">{vehicle.make} {vehicle.model} {vehicle.year}</p></Td><Td>{vehicle.customer.fullName}</Td><Td>{vehicle.vinNumber ?? "-"}</Td><Td>{vehicle.mileage.toLocaleString()} km</Td><Td>{vehicle.jobs.length}</Td></tr>)}
